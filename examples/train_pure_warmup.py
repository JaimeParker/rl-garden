"""State-based collect-only warmup with a pre-trained behavior policy.
...
class Args(WSRLTrainingArgs):
    ...
    warmup_steps: int = 0
    pure_warmup_steps: int = 50_000
    pure_warmup_epsilon: float = 0.0


def _validate_pure_warmup_args(args: Args) -> None:
    if args.load_checkpoint is None:
        raise ValueError(
            "Pure warmup requires --load_checkpoint pointing to a pre-trained behavior policy."
        )
    if args.num_offline_steps != 0 or args.offline_dataset_path is not None:
        raise ValueError(
            "Pure warmup is online-only: do not pass offline data or offline steps. "
            "Use examples/train_wsrl.py for WSRL offline-to-online training."
        )
    if args.offline_data_ratio != 0.0:
        raise ValueError("Pure warmup uses online replay only; offline_data_ratio must be 0.")
    if args.use_cql_loss or args.use_calql or args.online_use_cql_loss:
        raise ValueError("Pure warmup runs SAC-style online updates; CQL/CalQL must be off.")
    if args.warmup_steps != 0:
        raise ValueError("Pure warmup replaces WSRL warmup; warmup_steps must be 0.")
    if args.pure_warmup_steps <= 0:
        raise ValueError("pure_warmup_steps must be positive.")
    if args.num_online_steps <= args.pure_warmup_steps:
        raise ValueError(
            "num_online_steps includes the collect-only phase and must be larger "
            "than pure_warmup_steps to leave steps for online SAC updates."
        )
...
def main() -> None:
    ...
    _validate_pure_warmup_args(args)
    ...
    run_name = (
        args.exp_name
        or f"{args.env_id}__pure_warmup_state__{args.seed}__{int(time.time())}"
    )
    ...
    agent = WSRL(
        ...
        pure_warmup_steps=args.pure_warmup_steps,
        pure_warmup_epsilon=args.pure_warmup_epsilon,
    ...
    agent.load_prefill_policy_checkpoint(args.load_checkpoint, restore_global_step=True)
    source_step = agent._global_step
    agent.switch_to_online_mode(online_replay_mode="empty", offline_data_ratio=0.0)
    pure_warmup_end_step = source_step + args.pure_warmup_steps
    online_target_step = source_step + args.num_online_steps

    if args.std_log:
        print(
            "[pure_warmup] "
            f"behavior_checkpoint={args.load_checkpoint} "
            f"source_step={source_step} "
            f"online_start_step={agent._online_start_step} "
            f"pure_warmup_end_step={pure_warmup_end_step} "
            f"target_step={online_target_step} "
            f"epsilon={args.pure_warmup_epsilon}",
            flush=True,
        )
