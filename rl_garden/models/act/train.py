...
class SmallDemoDataset_ACTPolicy(Dataset): # Load everything into GPU memory
    def __init__(self, data_path, num_queries, device, num_traj):
        ...
        # When the robot reaches the goal state, actions are padded with "stay still".
        # If gripper_control=True, keep gripper command from the last step;
        # otherwise pad all action dims with zeros.
        if 'delta_pos' in args.control_mode or args.control_mode == 'base_pd_joint_vel_arm_pd_joint_vel' or args.control_mode == 'pd_ee_twist':
            self.action_dim = trajectories['actions'][0].shape[1]
            if args.gripper_control:
                self.pad_action_arm = torch.zeros((self.action_dim - 1,), device=device)
            else:
                self.pad_action_arm = torch.zeros((self.action_dim,), device=device)
        # else:
        ...
        self.delta_control = 'delta' in args.control_mode or args.control_mode == 'pd_ee_twist'
        ...
        if action_len < self.num_queries:
            if 'delta_pos' in args.control_mode or args.control_mode == 'base_pd_joint_vel_arm_pd_joint_vel' or args.control_mode == 'pd_ee_twist':
                if args.gripper_control:
                    ...
                    pad_action = self.pad_action_arm
...
class Agent(nn.Module):
    def __init__(self, env, args):
        super().__init__()
        print("env.single_observation_space: ", env.single_observation_space)
