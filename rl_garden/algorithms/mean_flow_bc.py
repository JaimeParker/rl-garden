...
``CombinedExtractor``, matching ``BC``/``FlowBC``'s convention exactly.
"""
...
class MeanFlowBC(OfflineRLAlgorithm):
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(obs_space, spaces.Box):
            if explicitly_set:
                raise ValueError(
                    "MeanFlowBC with Box observation space does not accept image-related "
            ...
            raise TypeError(
                f"MeanFlowBC supports Box or Dict observation spaces, got {type(obs_space)}"
            )
    ...
    def _default_features_extractor_class(self) -> type[BaseFeaturesExtractor]:
        ...
        raise TypeError(
            "MeanFlowBC supports Box or Dict observation spaces, got " + str(type(obs_space))
        )
