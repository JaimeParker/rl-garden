...
from rl_garden.envs.custom.agents.robots.panda import PandaWristCam
...
def _build_pose_axes(
    scene: ManiSkillScene, axis_half_length: float = 0.05, axis_half_width: float = 0.005
):
    """Create an RGB xyz-axis actor builder for visualizing a pose."""
    builder = scene.create_actor_builder()
    builder.initial_pose = sapien.Pose()
    builder.add_box_visual(
        pose=sapien.Pose([axis_half_length, 0, 0]),
        half_size=[axis_half_length, axis_half_width, axis_half_width],
        material=sapien.render.RenderMaterial(base_color=[1, 0, 0, 1]),
    )
    builder.add_box_visual(
        pose=sapien.Pose([0, axis_half_length, 0]),
        half_size=[axis_half_width, axis_half_length, axis_half_width],
        material=sapien.render.RenderMaterial(base_color=[0, 1, 0, 1]),
    )
    builder.add_box_visual(
        pose=sapien.Pose([0, 0, axis_half_length]),
        half_size=[axis_half_width, axis_half_width, axis_half_length],
        material=sapien.render.RenderMaterial(base_color=[0, 0, 1, 1]),
    )
    return builder


@register_env("PegInsertionSideNew-v1", max_episode_steps=100)
class PegInsertionSideEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    def _load_scene(self, options: dict):
        with torch.device(self.device):
            ...
            for i in range(self.num_envs):
                ...
                if self.debug_pose_vis:
                    # Debug visuals for peg-head pose / box-hole pose.
    ...
    def _update_pose_visualization(self):
        # goal_pose is fixed per reset, while peg_head_pose changes every step.
        if not self.debug_pose_vis:
            return
        if (
            not hasattr(self, "peg_head_pose_vis")
            or not hasattr(self, "box_hole_pose_vis")
        ):
            return
        self.peg_head_pose_vis.set_pose(self.peg_head_pose)
        self.box_hole_pose_vis.set_pose(self.box_hole_pose)
