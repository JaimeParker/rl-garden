#!/usr/bin/env bash
# RGBD PPO launcher with selectable image encoder.
set -euo pipefail
...
exec env RLG_STD_LOG="$STD_LOG" RLG_LOG_TYPE="$LOG_TYPE" RLG_LOG_KEYWORDS="$LOG_KEYWORDS" "$PYTHON_BIN" -u "$REPO_DIR/examples/train_ppo_rgbd.py" \
    --env_id PickCube-v1 \
    --obs_mode rgb \
    --num_envs 512 \
    --num_steps 50 \
